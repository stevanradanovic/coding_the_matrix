...
def create_voting_dict(strlist):
    ...
    """
    return {s.split()[0]: [int(i) for i in s.split()[3:]] for s in strlist}
...
def most_similar(sen, voting_dict):
    ...
    """
    return max({kee: policy_compare(sen, kee, voting_dict) for kee in voting_dict.keys() if kee != sen}.items(),
               key=itemgetter(1))[0]
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'c'
    """
    return min({kee: policy_compare(sen, kee, voting_dict) for kee in voting_dict.keys() if kee != sen}.items(),
               key=itemgetter(1))[0]
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return [sum([voting_dict[s][i] for s in sen_set]) / len(sen_set) for i in range(len(list(voting_dict.values())[0]))]
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    senator_names = voting_dict.keys()
    rez = sorted([(policy_compare(s1, s2, voting_dict), s1, s2)
                  for s1 in senator_names for s2 in senator_names if s1 != s2])
    return (rez[0][1], rez[0][2])
