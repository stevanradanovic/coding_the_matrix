...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    u.f = {kee: val for kee, val in u.f.items() if val != 0}
    v.f = {kee: val for kee, val in v.f.items() if val != 0}
    return u.f == v.f
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    uf = {kee: u.f[kee] if kee in u.f else 0 for kee in u.D}
    vf = {kee: v.f[kee] if kee in v.f else 0 for kee in u.D}
    af = {kee: uf[kee] + vf[kee] for kee in u.D if uf[kee] + vf[kee] != 0}
    return Vec(u.D, af)
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([u.f[kee] * v.f[kee] for kee, val in u.f.items() if kee in v.f])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {kee: val * alpha for kee, val in v.f.items() if val != 0})
