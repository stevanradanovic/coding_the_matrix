...
def movie_review(name):
    ...
    """
    return ['Five-stars!', 'Three thumbs up!'][randint(0, 1)]
...
def orSearch(inverseIndex, query):
    ...
    contains_any = set()
    for w in query:
        contains_any |= inverseIndex[w] if w in inverseIndex else {}
    return contains_any
...
def andSearch(inverseIndex, query):
    ...
    for k, v in inverseIndex.items():
        contains_all |= v
    for w in query:
        contains_all &= inverseIndex[w] if w in inverseIndex else {}
    return contains_all
